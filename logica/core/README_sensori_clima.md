...
| Ruolo | Entity ID CANONICO | Entità fisica | Note / sorgente |
| --- | --- | --- | --- |
| T interna zona giorno | **sensor.t_in_giorno** | sensor.t_in_giorno_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| T interna zona notte 1 | **sensor.t_in_notte1** | sensor.t_in_notte1_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| T interna zona notte 2 | **sensor.t_in_notte2** | sensor.t_in_notte2_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| T interna bagno | **sensor.t_in_bagno** | sensor.t_in_bagno_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| T esterna | **sensor.t_out** | sensor.t_out_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| UR interna zona giorno | **sensor.ur_in_giorno** | sensor.ur_in_giorno_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| UR interna zona notte 1 | **sensor.ur_in_notte1** | sensor.ur_in_notte1_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| UR interna zona notte 2 | **sensor.ur_in_notte2** | sensor.ur_in_notte2_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| UR interna bagno | **sensor.ur_in_bagno** | sensor.ur_in_bagno_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
| UR esterna | **sensor.ur_out** | sensor.ur_out_hw | Canonico; alias legacy `*_hw` da collegare al sensore reale. |
...
- **sensor.delta_t_in_out** / **sensor.delta_ah_in_out** — differenze in/outs usate per free-cooling, consigli finestre e hook AC.
...
- **Ventilazione/VMC**: `binary_sensor.vmc_sensors_ok`, `binary_sensor.vmc_anti_secco`, `binary_sensor.vmc_bagno_boost_auto`, `binary_sensor.vmc_freecooling_candidate`, `binary_sensor.vmc_freecooling_active`, `sensor.vmc_vel_target`, `sensor.vmc_vel_index`, `sensor.ventilation_priority` (**canonico**), `sensor.ventilation_reason` (**canonico**), `sensor.vmc_priority` (alias legacy), `sensor.vmc_reason` (alias legacy), `sensor.vmc_freecooling_status`, `sensor.clima_open_windows_recommended`.
...
- **AC**: `binary_sensor.ac_failsafe_sensors_bad`, `binary_sensor.ac_block_by_vmc`, `binary_sensor.ac_lock_min_on_ok`, `binary_sensor.ac_lock_min_off_ok`, `sensor.ac_priority` (**canonico**), `sensor.ac_reason` (**canonico**), `binary_sensor.stagione_calda` (vincolo stagionale per AC/ventilazione).
...
- **Finestre**: `binary_sensor.windows_all_closed` (**canonico aggregato**), `binary_sensor.windows_giorno_any`, `binary_sensor.windows_notte_any`, `binary_sensor.windows_bagno_any`, `sensor.windows_open_count`, `sensor.vent_finestre_state`; alias legacy temporanei `input_boolean.vent_finestra_*` da sostituire con `binary_sensor` fisici.

**Nota canonica/legacy**: usare sempre gli entity_id in grassetto come riferimento unico nei moduli climate. Gli alias legacy (`*_hw`, `sensor.vmc_priority`, `sensor.vmc_reason`, `input_boolean.vent_finestra_*`) esistono solo per compatibilità e vanno progressivamente rimossi una volta collegati i sensori fisici definitivi.
